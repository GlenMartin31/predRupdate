...
To illustrate how to achieve this with __predRupdate__, we here choose to apply model re-calibration to the existing logistic regression model shown in Example 1 above. Within __predRupdate__, we use the "pred_update()" function:
```{r}
# create a data.frame of the model coefficients, with columns being variables
...
We can then use "pred_stacked_regression()" function to apply stacked regression to aggregate these models into a single model for the new dataset:
```{r}
...
As shown above, the workflow within __predRupdate__ is broadly in two main stages: (i) input information about the existing CPM(s) using "pred_input_info()", and then (ii) apply validation, updating or model aggregation methods using "pred_validate()", "pred_update()" or "pred_stacked_regression()", respectively. Stage (i) requires specification of the 'model_info' parameter (which contains the information of the model parameters), and stage (ii) requires specification of the 'new_data' on which one wishes to apply the existing CPM(s). It is a requirement that there is consistency between the parameter/coefficient names given in the 'model_info' parameter of "pred_input_info()" and the names of variables in "new_data". Specifically, each coefficient named in 'model_info' should also be a column of 'new_data'. 

For example, the following results in an error, because the smoking variable passed through model_info in "pred_input_info()" no longer matches the name of the smoking variable in 'new_data' passed to "pred_validate()".
```{r}
# create a data.frame of the model coefficients, with columns being variables
coefs_table <- data.frame("Intercept" = -3.995, 
                          "Smoking" = 0.751)
...
Existing_Logistic_Model <- pred_input_info(model_type = "logistic",
                                           model_info = coefs_table)
try(pred_validate(Existing_Logistic_Model, 
                  new_data = SYNPM$ValidationData, 
                  binary_outcome = "Y"))
...
Here, `new_df` has Sex recorded as a factor variable with levels "M" and "F". Before working with functions in __predRupdate__, one must first convert any factor variables to indicator/dummy variables. We provide "dummy_vars()" within __predRupdate__ to assist with this. For example:
```{r}
